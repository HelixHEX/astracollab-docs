...
```bash
curl -X GET "https://api.astracollab.com/v1/files/file_1234567890abcdef" \
...
```json
{
  ...
  "data": {
    "file": {
      ...
      "downloadUrl": "https://storage.astracollab.com/files/file_1234567890abcdef?download=true&expires=3600",
      "folderId": "folder_123",
...
```bash
curl -X GET "https://api.astracollab.com/v1/files/file_1234567890abcdef?download=true" \
  -H "Authorization: Bearer your-api-key-here" \
  --output example.jpg
```
...
```bash
curl -X GET "https://api.astracollab.com/v1/files/file_1234567890abcdef?expires=3600" \
  -H "Authorization: Bearer your-api-key-here"
```
...
const downloadFile = async (fileId) => {
  const response = await fetch(`https://api.astracollab.com/v1/files/${fileId}?download=true`, {
    headers: {
...
const streamDownload = async (fileId) => {
  const response = await fetch(`https://api.astracollab.com/v1/files/${fileId}`, {
    headers: {
      'Authorization': 'Bearer your-api-key-here'
    }
  });
